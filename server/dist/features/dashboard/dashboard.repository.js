...
export class DashboardRepository {
    ...
    async getInventorySummary() {
        const [totalProducts, lowStockResult, outOfStock] = await Promise.all([
    ...
    async getTopSellingProducts() {
        ...
        return products
            ...
            quantitySold: product.deliveryItems.reduce((total, item) => total + item.quantity, 0),
        }))
    ...
    async getProductsInDemand() {
        ...
        return products
            ...
            productId: product.id,
            productName: product.name,
            quantityDemanded: product.orderItems.reduce((total, item) => total + item.quantity, 0),
        }))
            .sort((a, b) => b.quantityDemanded - a.quantityDemanded)
            .slice(0, 5);
    }
    async getRecentOrders() {
        ...
        return orders.map((order) => ({
            ...
            status: order.status,
            totalAmount: order.items.reduce((total, item) => total + Number(item.subtotal), 0),
        }));
    }
    ...
    async getDeliveries(filters) {
        ...
        return deliveries.map((delivery) => ({
            ...
            totalAmount: delivery.items.reduce((total, item) => total + Number(item.subtotal ?? 0), 0),
        }));
    }
