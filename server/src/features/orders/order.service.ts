...
export class OrderService {
  ...
  private validateId(id: number, resource: string) {
  ...
  async createOrder(data: {
    ...
    if (customer.status !== "ACTIVE") {
      throw new ValidationError(
        "Cannot create an order for an inactive customer",
      );
    }
  ...
  async updateOrder(
    ...
    if (data.customerId !== undefined) {
      ...
      if (customer.status !== "ACTIVE") {
        throw new ValidationError(
          "Cannot assign an inactive customer to an order",
        );
      }
    }
  ...
  if (order.status === "CONFIRMED" && order.delivery) {
    throw new ConflictError(
      "Cannot cancel an order that already has a delivery",
    );
  }
